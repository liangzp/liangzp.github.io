...
事实上我们想要知道的一个关键的值是每一台机器收益的期望。我们记$R_t$为t时刻我们的收益，$A_t$为我们在t时刻执行的动作，因此我们在t时刻执行一个动作a所期望得到的收益可以记做：$q(a)=E[R_{t}|A_{t}=a]$。但是由于我们无法得知这个值，因此我们需要在与环境交互的过程中估计这个值，我们记$Q_t(a)$为在t时刻的时候我们对于动作a的估值，我们希望在交互中更新这个$Q_{t}(a)$使得其最终充分接近$q(a)$。
