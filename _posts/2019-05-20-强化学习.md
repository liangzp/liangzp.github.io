...
![reward distribution](../img/intro to RL/Ch2/reward distribution.png)
...
一种常用的方法是采用均值来进行估计，即$Q_t(a)=\frac{\sum_{i=1}^{t-1}R_i\mathbb{I}_{A_i=a}}{\sum_{i=1}^{t-1} \mathbb{I}_{A_i=a}}$，由此衍生出的有greedy-method和$\epsilon$-greedy-method。前者每次只选择当前估计动作中最大的动作执行，即$A_t=argmax_aQ_t(a)$，后者则每次允许以概率$\epsilon$等概率随机选择所有动作，而只以1-$\epsilon$的概率来执行$A_t=argmax_aQ_t(a)$，explore的意义在于随着抽样的次数趋向于无穷，每一个bandit被抽样的样本数都会趋向无穷，因此对于每一个bandit的期望收益的估计则会是渐近收敛的。但是这两种方法的优劣还取决于任务的类型：如果reward的方差相对来说很大，那么$\epsilon$-greedy-method能够更加有效地搜索到最优的bandit，因此在run增加的时候能够趋向于以概率**大于**1-$\epsilon$的概率选择到最优的动作，而greedy-method有可能会一直只选择sub-optimal的动作。但是如果reward的方差很小，那么greedy-method能够很快地一直执行optimal的动作，而$\epsilon$-greedy-method则会永远有一定概率选择到了次优的动作。从下图可以看出，对于greedy-method由于其贪心的性质，策略在一开始的时候表现是最好的，但是由于其缺乏explore，因此一直选择的都是sub-optimal，故随着run的增加渐渐被$\epsilon$-greedy-method所超越。
![2.2](../../../Downloads/liangzp.github.io-master/img/intro to RL/Ch2/2.2.png)
...
![2.3](../img/intro to RL/Ch2/2.3.png)
...
![2.4](../img/intro to RL/Ch2/2.4.png)
...
###C![2.5](../img/intro to RL/Ch2/2.5.png)ontextual Bandits
